...
default_judge = Character(
    "Judge Wise",
    [],  # an external observer who is ANSWERING some of those questions
...
class Eval:
    ...
    ]  # list of (evaluator,comment) pairs for each long-form question
    ...
    def __repr__(self) -> str:
        allcomments = [
            f"Comments from {question} question:\n"
        ...
        if len(self) == 1:
            ...
            return (
                f"<Eval of {len(self)} dialogues: {repr(self.mean())}>"
...
def eval_by_participant(
    participant: Character, evaluee: str, dialogue: Dialogue
) -> Eval:
    ...
    warmup = (
        f"Hello {name}!  Here is a conversation that you had "
    ...
    scores = {}
    question = (
        f"Did {evaluee} listen carefully to you and engage with what you were saying?"
    )
    try:
        scores["engaged"] = agent.rating(d, research_team, question, 1, 5)
    except ValueError:
        pass  # will happen if LLM didn't return an integer in the range 1-5
...
def eval_by_observer(observer: Character, evaluee: str, dialogue: Dialogue) -> Eval:
    ...
    warmup = (
        f"Here is a conversation that you observed among {conjunction(speakers)}."
        f"\n\n{dialogue.script()}"
    ...
    try:
        scores["skilled"] = agent.rating(d, research_team, question, 1, 10)
    except ValueError:
        pass
...
def eval_dialogue(
    participant: Character, evaluee: str, judge: Character, dialogue: Dialogue
) -> Eval:
    """Combines `eval_by_particpant` and `eval_by_observer` into
    a single Eval that also includes a summary score."""
    e = (
        eval_by_participant(
            participant, evaluee, dialogue
        )  # creates EvaluationAgent(participant)
        + eval_by_observer(judge, evaluee, dialogue)
    )  # creates EvaluationAgent(judge)
    ...
    assert (
        e.n == 3
    )  # we summed 3 evals so e thinks there were 3 dialogues (maybe we should have specified n=1/3 for each!)
...
def eval_on_characters(
    ...
    log.warning(
        f"You just spent ${(ending_cost - starting_cost):.2f} of NLP money to evaluate {argubot}"
    )
