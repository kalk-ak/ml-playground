...
if not log.hasHandlers():  # avoid adding again when reimporting
    log.addHandler(
        RichHandler(
            level="NOTSET",
            markup=True,  # allows rich text log messages, with [color]s
            show_time=False,
            show_level=False,
        )
    )
log.setLevel(
    logging.WARNING
)  # usually WARNING, but you may want to change to INFO or DEBUG to get more output
...
def conjunction(
    ...
    ]  # if items was unordered, this imposes an order
    if len(strs) == 0:
        if zeroval is None:
...
def dialogue_to_openai(
    ...
    *,
    ...
    if speaker_names is None:
        speaker_names = len(speakers) > 2
    if compress is None:
        compress = len(speakers) > 2
    ...
    if compress:
        ...
        while i < len(openai_messages):
            if openai_messages[i]["role"] == "user":
                ...
                compressed = "\n\n".join(
                    [turn["content"] for turn in openai_messages[i:j]]
                )
                openai_messages[i:j] = [
                    {"role": "user", "content": f'"""\n{compressed}\n"""'}
                ]
...
class LLMAgent(Agent):
    ...
    def __init__(
        ...
        **kwargs,  # for both dialogue_to_openai and openai.chat.completions.create
        ...
        self.kwargs_format = {
            kw: kwargs[kw] for kw in kwargs if kw in kws_format
        }  # e.g., system
        self.kwargs_llm = {
            kw: kwargs[kw] for kw in kwargs if kw not in kws_format
        }  # e.g., temperature
    ...
    def response(self, d: Dialogue, **kwargs) -> str:
        ...
        kwargs_format = {kw: kwargs[kw] for kw in kwargs if kw in kws_format}
        kwargs_llm = {kw: kwargs[kw] for kw in kwargs if kw not in kws_format}

        messages = dialogue_to_openai(
            d, speaker=self.name, **(self.kwargs_format | kwargs_format)
        )

        pretty_messages = "\n".join(
            [
                f"[black on bright_yellow]({m['role']})"
                f"[/black on bright_yellow] {m['content']}"
                for m in messages
            ]
        )
        pretty_kws = (
            " with " + ", ".join(f"{key}={val}" for key, val in self.kwargs_llm.items())
            if self.kwargs_llm
            else ""
        )
        ...
        response = self.client.chat.completions.create(
            messages=messages, model=self.model, **(self.kwargs_llm | kwargs_llm)
        )
        ...
        if not isinstance(content, str):
            raise ValueError(
                "No content string returned from {self.kwargs_llm['client']}"
            )
...
class CharacterAgent(LLMAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if name is None:
            name = character.name  # by default, name the agent after the character
        if character.languages:
            langprefs = f", and you prefer to speak {conjunction(character.languages, conj='or')}"
        else:
            langprefs = ""
        system = (
            f"Your name is {character.name}{langprefs}. "
        ...
        self.conversation_starters = (
            character.conversation_starters
        )  # can be examined by simulate.simulated_dialogue
...
class EvaluationAgent(LLMAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        system = (
            f"Your name is {character.name} and you are {character.persona}."
        ...
        super().__init__(
            name, system=system, temperature=temperature, model=model, **kwargs
        )
        self.character = character
    ...
    def rating(
        self, d: Dialogue, speaker: str, question: str, lo: int, hi: int
    ) -> int | None:
        ...
        if not lo <= i <= hi:
            raise ValueError(
                "LLM agent generated rating {i} that's out of range [{lo}, {hi}]"
            )
        return i
...
for name, value in vars(characters).items():  # top-level attributes of the chars module
    if isinstance(value, Character):  # if it's a top-level var bound to a Character
        ...
        agent = CharacterAgent(
            char
        )  # make a CharacterAgent (might duplicate one from devset)
