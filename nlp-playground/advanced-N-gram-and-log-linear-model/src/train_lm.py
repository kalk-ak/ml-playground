...
from probs import (
    ...
    AddLambdaLanguageModel,
    BackoffAddLambdaLanguageModel,
    EmbeddingLogLinearLanguageModel,
    ImprovedLogLinearLanguageModel,
)
...
def get_model_filename(args: argparse.Namespace) -> Path:
    ...
    if args.smoother in (ADDLAMBDA, BACKOFF):
        ...
        return Path(
            f"{prefix}~lexicon={args.lexicon.name}~l2={args.l2_regularization}~epochs={args.epochs}.model"
        )
    else:
        raise NotImplementedError(
            f"Don't know how to construct filename for smoother {args.smoother}"
        )
...
def parse_args() -> Tuple[argparse.Namespace, argparse.ArgumentParser]:
    ...
    parser.add_argument(
        "smoother", type=str, help="Smoothing method", choices=SMOOTHERS
    )
    parser.add_argument(
        "train_file", type=Path, help="Training corpus (as a single file)"
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main():
    ...
    vocab = read_vocab(args.vocab_file)  # Reads the co
    if args.smoother == UNIFORM:
        ...
        lm = EmbeddingLogLinearLanguageModel(
            vocab, args.lexicon, args.l2_regularization, args.epochs
        )
    elif args.smoother == IMPROVED:
        if args.lexicon is None:
            parser.error(f"{args.smoother} requires a lexicon")
        lm = ImprovedLogLinearLanguageModel(
            vocab, args.lexicon, args.l2_regularization, args.epochs
        )
    else:
