...
class Dialogue(Tuple[Dict[str, str]]):
    ...
    def __rich__(self) -> str:
        ...
        return "\n".join(
            [
                f"[white on blue]({turn['speaker']})[/white on blue] {turn['content']}"
                for turn in self
            ]
        )
    ...
    def add(self, speaker: str, content: str) -> Dialogue:
        ...
        return Dialogue(
            itertools.chain(self, ({"speaker": speaker, "content": content},))
        )
    ...
    def rename(self, old: str, new: str) -> Dialogue:
        ...
        for turn in self:
            d = d.add(
                new if turn["speaker"] == old else turn["speaker"], turn["content"]
            )
        return d
    ...
    def __add__(self, other):
        if not isinstance(other, Dialogue):
            raise ValueError(
                f"Can only concatenate Dialogues with Dialogues, but got {type(other)}"
            )
