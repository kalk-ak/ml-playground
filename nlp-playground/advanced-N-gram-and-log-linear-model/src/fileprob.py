...
def main():
    ...
    total_log_prob: float = 0.0  # Stores the maximum probability of all the test_files
    tokens: float = 0.0
    for file in args.test_files:
        ...
        tokens += num_tokens(file)

    assert tokens != 0.0, (
        f"There are no tokens in the vocabulary. Check {args.test_file}"
    )
    ...
    tokens: float = sum(num_tokens(test_file) for test_file in args.test_files)
