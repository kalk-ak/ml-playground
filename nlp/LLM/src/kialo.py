...
class Kialo:
    ...
    def __init__(
        self,
        filenames: List[str],
        tokenizer: Optional[Callable[[str], List]] = tokenize_simple,
    ):
        ...
        self.parents: Dict[
            Kialo.Claim, Optional[Kialo.Claim]
        ] = {}  # maps each claim to its parent, or None in the case of a root
        self.pros: Dict[Kialo.Claim, List[Kialo.Claim]] = defaultdict(
            list
        )  # maps each claim to its "pro" children (initially [])
        self.cons: Dict[Kialo.Claim, List[Kialo.Claim]] = defaultdict(
            list
        )  # maps each claim to its "con" children (initially [])
        self.claims: Dict[
            str, List[str]
        ] = {}  # maps a kind of claim to the list of all claims of that kind
        self.bm25: Dict[
            str, BM25_Index
        ] = {}  # maps a kind of claim to a BM25 index on claims of that kind
    ...
    def add_discussion(self, filename: str):
        ...
        index: Dict[
            str, Kialo.Claim
        ] = {}  # map claim id to actual claim; ids will be discarded after reading file
        metadata_re = re.compile(
            "(((\\d+\\.)*)\\d+.)\\s+(Thesis|Pro|Con):"
        )  # matches metadata format
        xref_re = re.compile("-> See ((\\d+\\.)+)")  # matches cross-reference format
        with open(filename, encoding="utf-8") as f:
            id, id_parent, polarity = (
                None,
                None,
                None,
            )  # vars maintained throughout loop below
            for line in f:
                ...
                if match:  # this line gives metadata that we'll remember for a claim on the next line
                    ...
                    if not (id_parent == "") == (polarity == "Thesis"):
                        raise ValueError(
                            f"Thesis should appear at root of tree and nowhere else: {line}"
                        )
                else:
                    if id is not None:  # previous line gave us metadata about a claim
                        ...
                        if match:
                            line = index[
                                match.group(1)
                            ]  # will raise KeyError if referenced claim didn't appear earlier in file
                        else:
                            # remove any bracketed footnote numbers or footnoted page refs
                            line = re.sub(" \\(p[^)]* \\[\\d+\\]\\)", "", line)
                            line = re.sub(" \\[\\d+\\]", "", line)

                        index[id] = claim = Kialo.Claim(
                            line
                        )  # remember it by numeric id for later lookup while processing this file
                    ...
                    id, id_parent, polarity = (
                        ...
                        None,
                        None,
                    )  # this line did not give metadata
    ...
    def random_chain(self, n: int = 1):
        ...
        claim = random.choice(
            [claim for claim in self.parents if self.depth(claim) >= n]
        )
    ...
    def closest_claims(
        self, s: str, n: int = 1, kind: str = "all"
    ) -> List["Kialo.Claim"]:
        ...
        if self.tokenizer is None:
            raise ValueError(
                "Can't call this method on a Kialo object that doesn't have a tokenizer"
            )
        ...
        if kind not in self.claims:
            ...
            if kind == "all":  # all claims
                ...
                self.claims[kind] = [
                    claim for claim in self.parents if self.cons[claim]
                ]
            elif kind == "has_pros":  # all claims that have at least one "pro" response
                self.claims[kind] = [
                    claim for claim in self.parents if self.pros[claim]
                ]
            else:
            ...
            if self.claims[
                kind
            ]:  # only build if there are some claims, to avoid div by 0
                self.bm25[kind] = BM25_Index(
                    self.claims[kind], tokenizer=self.tokenizer
                )
