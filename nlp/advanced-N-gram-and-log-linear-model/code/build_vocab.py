...
from probs import Wordtype, EOS, OOV, BOS, read_tokens
...
def build_vocab(*files: Path, threshold: int) -> Set[str]:
    ...
    sys.stderr.write(
        f"Vocabulary size is {len(vocab)} types including OOV, EOS, and BOS\n"
    )
    return vocab
...
def main():
    ...
    # If documents were passed as arguments, use them.
    # Otherwise, read the list of file paths from standard input.
    if args.documents:
        document_paths = args.documents
    elif not sys.stdin.isatty():
        # We are receiving piped input.
        document_paths = [Path(line.strip()) for line in sys.stdin if line.strip()]
    else:
        # No files were provided either way.
        sys.stderr.write(
            "Error: No document paths provided either as arguments or via standard input.\n"
        )
        sys.exit(1)

    vocab = build_vocab(*document_paths, threshold=args.threshold)
